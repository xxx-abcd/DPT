...
def loadModel(sparse_trn_mats, ori_mats):
    ...
    with torch.no_grad():
        ...
        for beh in range(len(behaviors) - 1):
            ...
            generated_graphs[beh] = act(user_embed @ beh_w @ item_embed.t())  # 正常要用这个

            # generated_graphs[beh] = act(user_embeds[beh] @ beh_w @ item_embeds[beh].t())
            # generated_graphs[beh] = act(user_embeds[beh] @ item_embeds[beh].t())
