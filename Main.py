...
def model_prepare(int_types, item_in, item_num, item_out, tr_mats, tr_matsT, user_in, user_num, labelP,
    ...
    if args.prompt and not args.denoise_tune:
        ...
        if args.wsdm:
            ...
            for i, p in enumerate(net_dict):
                if str(p) in frozen_para: # 这种的不继承GCN参数
                    net_dict[str(p)].copy_(pre_dict[str(p)])
                # net_dict[str(p)].copy_(pre_dict[str(p)])
...
def sampleTestBatch(batch_user_id, batch_item_id, trainMat_target, labelP):
    ...
    cur = 0
    for i in range(batch):  # 我认为就是对遍历每个user
        pos_item = user_item1[i]  # user purchase过的item
        negset = np.reshape(np.argwhere(sub_trainMat[i] == 0), [-1])  # user没交互过的item id
        pvec = labelP[negset]  # user 交互过多少个item，
        pvec = pvec / np.sum(pvec)

        random_neg_sam = np.random.permutation(negset)[:99]  # 对negset随机排序，取前100个
        # random_neg_sam = negset
        # if i==0:
        #     print('len:negset',len(negset))
